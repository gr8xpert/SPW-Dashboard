...
class ClientCustomPropertyTypeGroup extends Model
{
    protected $fillable = [
        'client_id',
        'name',
    ...
    {
        return $this->hasMany(ClientPropertyTypeMapping::class, 'custom_group_id')->orderBy('sort_order');
    }
    ...
    protected static function nestGroups($groups, array $feedTypes = []): array
    {
        ...
        foreach ($groups as $group) {
            $item = [
                'id' => 'custom_type_group_' . $group->id,
                'group_id' => $group->id,
                'name' => $group->name,
                'slug' => $group->slug,
                'type' => 'custom_group',
