    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <div class="flex-grow-1">
            <div class="fw-medium">
                ...
                @endif
                @if($group->parent_feed_type_id)
                    <span class="badge bg-info" title="Nested under {{ $group->parent_feed_type_name }}">
                        <i class="bi bi-diagram-3"></i> {{ $group->parent_feed_type_name }}
                    </span>
                @endif
            </div>
            <small class="text-muted">{{ $group->mappings->count() }} property type(s)</small>
        </div>
        <div class="btn-group btn-group-sm">
            <button type="button" class="btn btn-outline-secondary btn-sm"
                    onclick="editGroup({{ $group->id }}, '{{ addslashes($group->name) }}', {{ $group->parent_group_id ?? 'null' }}, '{{ $group->parent_feed_type_id ?? '' }}', '{{ addslashes($group->description ?? '') }}', {{ $group->is_active ? 'true' : 'false' }})">
                <i class="bi bi-pencil"></i>
            </button>
            <form method="POST" action="{{ route('admin.widget-clients.property-type-grouping.groups.destroy', [$client, $group]) }}"
                  onsubmit="return confirm('Delete this group? Mapped property types will become unmapped.')" class="d-inline">
                @csrf
                @method('DELETE')
...
@foreach($group->children as $child)
    @include('admin.widget-clients._property-type-group-item', ['group' => $child, 'level' => $level + 1])
@endforeach
