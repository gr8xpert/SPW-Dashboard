...
@section('page-title', 'Feature Grouping - ' . $client->domain)

@section('page-content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">Feature Grouping</h1>
            <p class="text-muted mb-0">{{ $client->domain ?: $client->company_name }}</p>
        </div>
        <div class="d-flex gap-2">
            <form method="POST" action="{{ route('admin.widget-clients.feature-grouping.toggle', $client) }}">
                @csrf
                <button type="submit" class="btn btn-{{ $client->custom_feature_grouping_enabled ? 'success' : 'outline-secondary' }}">
                    <i class="bi bi-{{ $client->custom_feature_grouping_enabled ? 'check-circle-fill' : 'circle' }} me-1"></i>
                    {{ $client->custom_feature_grouping_enabled ? 'Enabled' : 'Disabled' }}
                </button>
            </form>
    ...
    @if(!$client->custom_feature_grouping_enabled)
        <div class="alert alert-info">
            ...
            Custom feature grouping is disabled. Enable it to create custom groups and map features.
        </div>
    @endif
    ...
    <div class="row">
                    ...
                    @else
                        <div class="list-group list-group-flush" id="groups-list">
                            @foreach($groups as $group)
                                @include('admin.widget-clients._feature-group-item', ['group' => $group, 'level' => 0])
                            @endforeach
                        </div>
                    @endif
                </div>
        ...
        {{-- Right Panel: Unmapped Features --}}
        <div class="col-lg-6 mb-4">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white border-bottom pt-4 pb-3">
                    <h6 class="fw-bold mb-0"><i class="bi bi-check2-square me-2 text-primary"></i>Unmapped Features</h6>
                    <div class="mt-2">
                        <input type="text" class="form-control form-control-sm" id="feature-search" placeholder="Search features...">
                    </div>
                </div>
                <div class="card-body p-0" style="max-height: 600px; overflow-y: auto;">
                    @if(empty($unmappedFeatures))
                        <div class="p-4 text-center text-muted">
                            <i class="bi bi-check-circle display-4 d-block mb-2"></i>
                            All features are mapped.
                        </div>
                    @else
                        <div class="list-group list-group-flush" id="unmapped-features">
                            @foreach($unmappedFeatures as $feature)
                                <label class="list-group-item list-group-item-action d-flex align-items-center gap-2 feature-item"
                                       data-name="{{ strtolower($feature['full_name']) }}">
                                    <input type="checkbox" class="form-check-input feature-checkbox" value="{{ $feature['id'] }}"
                                           data-name="{{ $feature['name'] }}">
                                    <div class="flex-grow-1">
                                        <div class="fw-medium">{{ $feature['name'] }}</div>
                                        @if($feature['full_name'] !== $feature['name'])
                                            <small class="text-muted">{{ $feature['full_name'] }}</small>
                                        @endif
                                    </div>
                                </label>
                            @endforeach
                ...
                @if(!empty($unmappedFeatures) && !$groups->isEmpty())
...
<div class="modal fade" id="createGroupModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.widget-clients.feature-grouping.groups.store', $client) }}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="group-name" name="name" required placeholder="e.g., Outdoor Features">
                    </div>
                    <div class="mb-3">
                        <label for="parent-feed-feature" class="form-label">Nest Under Feature Category</label>
                        <select class="form-select" id="parent-feed-feature" name="parent_feed_feature_id" onchange="updateParentName(this, 'parent-feed-feature-name')">
                            <option value="">None (Show at top level)</option>
                            @foreach($parentableFeatures as $pfeature)
                                <option value="{{ $pfeature['id'] }}" data-name="{{ $pfeature['name'] }}">{{ $pfeature['full_name'] }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" id="parent-feed-feature-name" name="parent_feed_feature_name" value="">
                        <div class="form-text">Select a feature category to nest this group under it.</div>
                    </div>
                    <div class="mb-3">
        ...
        <div class="modal-content">
            <form method="POST" id="edit-group-form">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="edit-parent-feed-feature" class="form-label">Nest Under Feature Category</label>
                        <select class="form-select" id="edit-parent-feed-feature" name="parent_feed_feature_id" onchange="updateParentName(this, 'edit-parent-feed-feature-name')">
                            <option value="">None (Show at top level)</option>
                            @foreach($parentableFeatures as $pfeature)
                                <option value="{{ $pfeature['id'] }}" data-name="{{ $pfeature['name'] }}">{{ $pfeature['full_name'] }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" id="edit-parent-feed-feature-name" name="parent_feed_feature_name" value="">
                        <div class="form-text">Select a feature category to nest this group under it.</div>
                    </div>
                    <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const reorderGroupsUrl = '{{ route("admin.widget-clients.feature-grouping.groups.reorder", $client) }}';
    const reorderMappingsBaseUrl = '{{ route("admin.widget-clients.feature-grouping.index", $client) }}/groups';
    ...
    if (mapBtn) {
        mapBtn.addEventListener('click', function() {
            ...
            form.method = 'POST';
            form.action = '{{ route("admin.widget-clients.feature-grouping.groups.store", $client) }}'.replace('/groups', '/groups/' + groupId + '/map');
            ...
            selected.forEach(function(feature, i) {
                ['id', 'name'].forEach(function(key) {
                    const input = document.createElement('input');
                    input.type = 'hidden';
                    input.name = 'features[' + i + '][' + key + ']';
    ...
    // Edit group modal
    window.editGroup = function(id, name, parentId, parentFeedFeatureId, description, isActive) {
        document.getElementById('edit-group-form').action = '{{ route("admin.widget-clients.feature-grouping.index", $client) }}/groups/' + id;
        document.getElementById('edit-group-name').value = name;
        document.getElementById('edit-parent-group').value = parentId || '';
        document.getElementById('edit-parent-feed-feature').value = parentFeedFeatureId || '';
        ...
        updateParentName(document.getElementById('edit-parent-feed-feature'), 'edit-parent-feed-feature-name');
